...
export function WindArrow({ dir, size }: { dir: number; size: number }) {
  return (
...
// Weericoon uit de WMO-code (via wxGroup). Lijnicoon, 1.6px.
export function WxIcon({ code, size }: { code: number | null; size: number }) {
  const g: WxGroup = wxGroup(code);
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.6} strokeLinecap="round" aria-hidden style={{ flex: "0 0 auto" }}>
      {g === "clear" ? SUN
